function SankeyChart(data,options)
    ...
    for c = 1:max_n_cats_layer
        ...
        if show_perc
            if val > 0
                ...
                if t == n_layers
                    text(X(t)-0.07, bars{t}(c)/2 + sum(bars{t}(1:c-1)) + (c-1)*gap, txt2print,'HorizontalAlignment','right','Color',text_color)
                else
                    text(X(t)+0.07, bars{t}(c)/2 + sum(bars{t}(1:c-1)) + (c-1)*gap, txt2print,'HorizontalAlignment','left','Color',text_color)
                end
            end
        else
            if show_cat_labels
                if val > 0
                if t == n_layers
                    text(X(t)-0.07, bars{t}(c)/2 + sum(bars{t}(1:c-1)) + (c-1)*gap, layer_cat_names{t}{c},'HorizontalAlignment','right','Color',text_color)
                else
                    text(X(t)+0.07, bars{t}(c)/2 + sum(bars{t}(1:c-1)) + (c-1)*gap, layer_cat_names{t}{c},'HorizontalAlignment','left','Color',text_color)
                end
                end
            end
        end
